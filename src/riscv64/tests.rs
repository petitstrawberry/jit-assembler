use crate::riscv64::{csr, reg, Instruction, Riscv64InstructionBuilder};
...
fn compare_instruction(jit_instr: Instruction, gnu_assembly: &str) {
    ...
    assert!(
        gnu_bytes.len() >= 4,
        "GNU assembler output should be at least 4 bytes"
    );
...
fn test_issue_requested_csrs() {
    ...
    assert_eq!(
        instructions.len(),
        18,
        "Should have 18 CSR instructions (10 M-mode + 8 S-mode)"
    );
    ...
    let write_instructions = builder2.instructions().unwrap();
...
fn test_comprehensive_alias_demo() {
    ...
    println!(
        "Generated {} instructions using aliases",
        instructions.len()
    );
    for (i, instr) in instructions.iter().enumerate() {
        println!("  {}: {}", i, instr);
...
fn test_jump_instruction_encoding_correctness() {
    ...
    let instructions4 = builder4.instructions().unwrap();
    ...
    let instructions5 = builder5.instructions().unwrap();
...
fn test_jump_branch_edge_cases() {
    ...
    let instructions6 = builder6.instructions().unwrap();
...
fn compare_instructions(jit_instrs: &[Instruction], gnu_assembly: &str) {
    ...
    assert_eq!(
        jit_bytes.len(),
        jit_instrs.len() * 4,
        "JIT instructions should be 4 bytes each"
    );
    assert!(
        gnu_bytes.len() >= jit_bytes.len(),
        "GNU assembler output should be at least {} bytes",
        jit_bytes.len()
    );
...
fn test_binary_correctness_multiline_macro_comparison() {
    ...
    let builder_instructions = builder.instructions().unwrap();

    // Both should match
    assert_eq!(macro_instructions.len(), builder_instructions.len());
    for (i, (macro_instr, builder_instr)) in macro_instructions
        .iter()
        .zip(builder_instructions.iter())
        .enumerate()
    {
        assert_eq!(
            macro_instr.bytes(),
            builder_instr.bytes(),
            "Instruction {} differs between macro and builder",
            i
        );
    }
...
fn test_jit_constant_function() {
    ...
    assert!(
        jit_func.is_ok(),
        "JIT constant function creation should succeed"
    );
}
...
fn test_jit_function_chaining() {
    ...
    assert!(
        result.is_ok(),
        "Chained JIT function creation should succeed"
    );
}
