...
impl<R: Register> fmt::Display for RegisterUsageInfo<R> {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        write!(
            f,
            "RegisterUsage(total: {}, caller-saved: {}, callee-saved: {}, special: {})",
            self.register_count(),
            caller_saved,
            callee_saved,
            special
...
mod tests {
    ...
    fn test_abi_class_display() {
        assert_eq!(
            crate::common::AbiClass::CallerSaved.to_string(),
            "caller-saved"
        );
        assert_eq!(
            crate::common::AbiClass::CalleeSaved.to_string(),
            "callee-saved"
        );
