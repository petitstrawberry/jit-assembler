...
fn test_binary_correctness_move_immediate() {
    ...
    if !instructions.is_empty() {
        ...
        assert!(
            instructions.len() >= 1,
            "Should generate at least one instruction for large immediate"
        );
    }
}
...
fn test_move_instructions_comprehensive() {
    ...
    assert!(
        instructions.len() >= 5,
        "Should generate at least 5 instructions"
    );
    ...
    assert_eq!(
        (mov_value >> 5) & 0x1F,
        31,
        "MOV should use XZR as source register"
    );
}
...
fn test_control_flow_instructions_comprehensive() {
    ...
    assert_eq!(
        (ret_default_value >> 5) & 0x1F,
        30,
        "Default RET should use X30 (LR)"
    );
...
fn test_immediate_value_ranges() {
    ...
    assert_eq!(
        (addi_max_value >> 10) & 0xFFF,
        4095,
        "Should encode maximum 12-bit immediate"
    );

    let movz_max = instructions[3];
    let movz_max_value = movz_max.value();
    assert_eq!(
        (movz_max_value >> 5) & 0xFFFF,
        0xFFFF,
        "Should encode maximum 16-bit immediate"
    );
}
...
fn test_register_combinations() {
    ...
    assert_eq!(
        (zero_reg_value >> 5) & 0x1F,
        31,
        "Should use XZR (31) as Rn"
    );
...
fn test_jit_large_immediate_values() {
    ...
    for (value, description) in test_cases {
        ...
        assert_eq!(
            result, value,
            "JIT execution failed for {}: expected 0x{:016x}, got 0x{:016x}",
